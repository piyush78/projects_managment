class TasksController < ApplicationController
  ...
  def priority_task
    @task = Task.find(params[:id])
    @task.update_attributes(priority: params[:priority])
  end 

  def task_done
    @task = Task.find(params[:id])
    @task.update_attributes(task_done: params[:task_done])
  end
  ...
  private
    ...
    def task_params
      params.require(:task).permit(:title, :description, :deadline, :priority, :task_done)
    end
end
